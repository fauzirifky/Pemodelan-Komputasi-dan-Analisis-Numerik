...
\item[\textbf{B4.4 (L4)}] Sesuai Lembar Kerja, gunakan data $ (1,0.5),(2,2.5),(3,2.0),(4,4.0),(5,3.5)$. (a) Cari model regresi linear $\widehat y=a+bx$ dengan OLS, turunkan SPL normalnya. (b) \textbf{Tentukan model nonlinear:} pada \emph{data yang sama} fit $\widehat y=a_0(1-e^{-a_1x})$, bentuk Jacobian parameter $Z$, lalu jalankan Gauss--Newton. (c) Hitung MSE, RMSE, MAPE, $R^2$ dan buat grafik data-model, residual serta \texttt{errs}. Bandingkan pula dengan contoh data kelas di bagian sebelumnya.
\item[\textbf{B4.5 (L5)}] Gunakan dua belas data saturasi pada tabel berikut. Model \textbf{default} adalah $\widehat y=a_0(1-e^{-a_1x})$. Hitung parameter dan keempat metrik serta grafik \texttt{errs}. Untuk tantangan, coba model saturasi rasional $\widehat y=c_0x/(c_1+x)$ pada \textbf{data yang sama}; turunkan Jacobian parameter dan bandingkan metriknya. Jelaskan perbedaan bentuk model dan batasan parameter. Untuk mempertahankan butir asli B4.5, tuliskan juga objektif $S(a,b)=\sum_i(y_i-ae^{bx_i})^2$ dan kondisi optimalitas $\partial_a S=\partial_b S=0$ untuk model eksponensial $ae^{bx}$.

\item[\textbf{B4.6 (L6)}]  Gunakan fungsi $\widehat y=ae^{bx}$ pada latihan B4.5, gunakan data sintetis \emph{berbeda} berikut:
\begin{center}\small
...
\end{tabular}
\end{center}
Sebagai tantangan lain, bandingkan dengan fungsi pangkat $f(x;c,d)=cx^d$ (untuk $x>0$).
