...
\section{Pola kerja driver dan function}
\textbf{Driver} adalah skrip yang menentukan masalah, data, toleransi, memanggil function, dan menggambar grafik. \textbf{Function} menerima input formal, menghitung, dan mengembalikan output tanpa menghapus workspace pengguna. Pisahkan keduanya dalam file \texttt{.m} berbeda; nama file function harus sesuai nama fungsi.

Contoh file \texttt{relative\_error.m} berikut digunakan di berbagai bab:
\lstinputlisting[language=Matlab]{matlab/relative_error.m}
...
\item[\textbf{B0.3 (L3)}] Suatu iterasi menghasilkan urutan hampiran \(2.0000,\ 1.7500,\ 1.7143,\ 1.7321\). Hitung perubahan relatif antariterasi dan bandingkan formula biasa dengan formula berskala yang menggunakan \(\max(1,|x_{\mathrm{new}}|)\).
