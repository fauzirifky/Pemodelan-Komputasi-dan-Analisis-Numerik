...
\section{Program MATLAB: function dan driver}
\textbf{Function} \texttt{jacobi.m} dan \texttt{gauss\_seidel.m} menerima $A,b,x_0,\tau,N_{\max}$ lalu mengembalikan solusi dan \texttt{history}. \textbf{Driver} \texttt{driver\_spl.m} menyiapkan sistem dan solusi pembanding, memanggil kedua function, menampilkan tabel dan membuat grafik galat serta residual.

\MatlabFile{jacobi.m (function)}
\lstinputlisting[language=Matlab]{matlab/jacobi.m}
\MatlabFile{gauss\_seidel.m (function)}
\lstinputlisting[language=Matlab]{matlab/gauss_seidel.m}
\MatlabFile{driver\_spl.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_spl.m}

\Note{Jacobi membaca seluruh komponen \texttt{xold}; Gauss-Seidel membaca komponen kiri dari \texttt{xnew} dan kanan dari \texttt{xold}. Input dipaksa menjadi vektor kolom agar dimensi perhitungan konsisten.}
