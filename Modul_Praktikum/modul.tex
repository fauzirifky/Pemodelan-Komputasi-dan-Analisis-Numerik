...
B4.1--B4.5 & Praktikum 3 & Newton multivariabel, OLS, Gauss--Newton, MSE, RMSE, MAPE, $R^2$\\
...
\section*{Pola penulisan function dan driver}
Setiap praktikum memakai dua jenis file. \textbf{Function} menyimpan algoritma umum yang menerima input dan mengembalikan hasil serta riwayat. \textbf{Driver} menyiapkan data, model, parameter, memanggil function, lalu membuat tabel dan visualisasi. Jalankan skrip driver dari folder \path{Modul_Praktikum/matlab}. Jangan menaruh \texttt{clear} di dalam function numerik. Notasi \texttt{err} dan \texttt{errs} menyatakan rasio desimal, bukan persen.

\noindent\textbf{Konsistensi lintas bab:} akar dan SPNL memakai grafik perubahan parameter/solusi terhadap iterasi; SPL memakai galat serta residual terhadap iterasi; turunan/integral/PDB memakai galat terhadap ukuran langkah atau jumlah grid ketika algoritma bukan iterasi akar. Selalu beri label sumbu dan toleransi yang dipakai.
...
% ================================================================
\input{chapters/00_matlab_dasar}

\input{chapters/01_akar}

\input{chapters/02_spl}

\input{chapters/03_spnl_curvefit}
...
\section{Program MATLAB: pisahkan function dan driver}
\textbf{Driver:} parameter/data dan grafik. \textbf{Function:} algoritma numerik yang menerima input dan mengembalikan output. Semua file berada pada \path{Modul_Praktikum/matlab}.

\MatlabFile{numdiff.m (function)}
\lstinputlisting[language=Matlab]{matlab/numdiff.m}

\MatlabFile{driver\_turunan.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_turunan.m}
...
\section{Luaran driver dan grafik galat}
Jalankan driver dan tampilkan keluaran numerik, nilai acuan bila tersedia, serta grafik berlabel lengkap. Plot galat terhadap ukuran langkah $h$ atau banyak subinterval $n$; simpan nilainya sebagai \texttt{errs} pada driver bila diperlukan.
...
\MatlabFile{driver\_richardson.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_richardson.m}
...
\MatlabFile{trap\_comp.m (function)}
\lstinputlisting[language=Matlab]{matlab/trap_comp.m}

\MatlabFile{midpoint\_comp.m (function)}
\lstinputlisting[language=Matlab]{matlab/midpoint_comp.m}

\MatlabFile{simpson\_comp.m (function)}
\lstinputlisting[language=Matlab]{matlab/simpson_comp.m}

\MatlabFile{driver\_integral.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_integral.m}
...
\MatlabFile{romberg\_fun.m (function)}
\lstinputlisting[language=Matlab]{matlab/romberg_fun.m}

\MatlabFile{trap\_comp.m (function)}
\lstinputlisting[language=Matlab]{matlab/trap_comp.m}

\MatlabFile{driver\_romberg.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_romberg.m}
...
\subsection*{Metode Heun (prediktor--korektor)}
Dua evaluasi kemiringan meningkatkan akurasi terhadap Euler. Prediktor dan korektor adalah
\begin{align}
\widetilde y_{n+1}&=y_n+h f(t_n,y_n),\\
y_{n+1}&=y_n+\frac h2\bigl[f(t_n,y_n)+f(t_n+h,\widetilde y_{n+1})\bigr].
\end{align}
Metode Heun inilah yang digunakan pada B7.2 dan Mock-Up UAS.3, sehingga tidak cukup hanya menampilkan Euler dan RK4.
...
\MatlabFile{euler\_ivp.m (function)}
\lstinputlisting[language=Matlab]{matlab/euler_ivp.m}

\MatlabFile{heun\_ivp.m (function)}
\lstinputlisting[language=Matlab]{matlab/heun_ivp.m}

\MatlabFile{rk4\_ivp.m (function)}
\lstinputlisting[language=Matlab]{matlab/rk4_ivp.m}

\MatlabFile{driver\_ode.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_ode.m}


\section{Luaran driver dan grafik galat}
Jalankan driver dan tampilkan keluaran numerik, nilai acuan bila tersedia, serta grafik berlabel lengkap. Bila metode mengulang dalam waktu, plot galat terhadap waktu dan ulangi eksperimen dengan beberapa $h$.
...
\MatlabFile{driver\_stabilitas.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_stabilitas.m}
...
\MatlabFile{bisection\_fun.m (function)}
...
\MatlabFile{driver\_terpadu.m (driver)}
\lstinputlisting[language=Matlab]{matlab/driver_terpadu.m}
...
\noindent\textbf{File utama edisi ini:}\par
\begin{itemize}
\item Akar dan SPL: \texttt{driver\_akar.m}, \texttt{driver\_spl.m}.
\item SPNL dan regresi: \texttt{driver\_spnl.m}, \texttt{driver\_regresi.m}, \texttt{driver\_tugas\_curvefit.m}.
\item Turunan dan integral: \texttt{driver\_turunan.m}, \texttt{driver\_richardson.m}, \texttt{driver\_integral.m}, \texttt{driver\_romberg.m}.
\item PDB dan sintesis: \texttt{driver\_ode.m}, \texttt{driver\_stabilitas.m}, \texttt{driver\_terpadu.m}.
\end{itemize}
Function yang dipakai dijelaskan di setiap bab. Fungsi tambahan antara lain \texttt{relative\_error.m}, \texttt{regresi\_linear.m}, \texttt{regresi\_nonlinear.m}, serta \texttt{metrik\_regresi.m}. Nama file lama masih disimpan sebagai kompatibilitas; jalankan driver baru sesuai bab.
